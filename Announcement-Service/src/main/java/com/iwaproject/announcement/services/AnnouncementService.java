...
public class AnnouncementService {
     ...
     */
    public Announcement createAnnouncementFromDto(final AnnouncementRequestDto requestDto) {
        // Find care type by label
        CareType careType = careTypeRepository.findByLabel(requestDto.getCareTypeLabel())
                .orElseThrow(() -> new IllegalArgumentException(
                        "Care type not found with label: " + requestDto.getCareTypeLabel()));

        // Convert DTO to entity
        Announcement announcement = announcementMapper.toEntity(requestDto, careType);
     ...
     * @throws IllegalArgumentException if the care type does not exist
     */
    public Announcement createAnnouncement(final Announcement announcement) {
        // Validate that the care type exists
        if (announcement.getCareType() != null && announcement.getCareType().getId() != null) {
            CareType careType = careTypeRepository.findById(announcement.getCareType().getId())
                    .orElseThrow(() -> new IllegalArgumentException("Care type not found with id: " + announcement.getCareType().getId()));
            announcement.setCareType(careType);
        } else {
            throw new IllegalArgumentException("Care type is required");
        }
     ...
     * @throws IllegalArgumentException if the announcement or care type does not exist
     */
    public Announcement updateAnnouncement(Long id, Announcement updatedAnnouncement) {
        ...
        if (updatedAnnouncement.getLocation() != null) {
            existingAnnouncement.setLocation(updatedAnnouncement.getLocation());
        }
        ...
        if (updatedAnnouncement.getSpecificInstructions() != null) {
            existingAnnouncement.setSpecificInstructions(updatedAnnouncement.getSpecificInstructions());
        }
        ...
        if (updatedAnnouncement.getVisitFrequency() != null) {
            existingAnnouncement.setVisitFrequency(updatedAnnouncement.getVisitFrequency());
        }
        if (updatedAnnouncement.getRemuneration() != null) {
            existingAnnouncement.setRemuneration(updatedAnnouncement.getRemuneration());
        }
        if (updatedAnnouncement.getIdentityVerificationRequired() != null) {
            existingAnnouncement.setIdentityVerificationRequired(updatedAnnouncement.getIdentityVerificationRequired());
        }
        if (updatedAnnouncement.getUrgentRequest() != null) {
            existingAnnouncement.setUrgentRequest(updatedAnnouncement.getUrgentRequest());
        }

        // Update care type if provided
        if (updatedAnnouncement.getCareType() != null && updatedAnnouncement.getCareType().getId() != null) {
            CareType careType = careTypeRepository.findById(updatedAnnouncement.getCareType().getId())
                    .orElseThrow(() -> new IllegalArgumentException("Care type not found with id: " + updatedAnnouncement.getCareType().getId()));
     ...
     */
    public Announcement changeAnnouncementStatus(Long id, AnnouncementStatus newStatus) {
     ...
     * @param ownerId the owner id
     * @param status the announcement status
     * @return list of announcements for the owner with the specified status
     */
    @Transactional(readOnly = true)
    public List<Announcement> getAnnouncementsByOwnerIdAndStatus(Long ownerId, AnnouncementStatus status) {
        return announcementRepository.findByOwnerIdAndStatus(ownerId, status);
    }
}
